...
function bbpress_livesearch_return_data()
{
	...
	$title = sanitize_text_field($_GET['bbp_topic_title']);
	$title = str_replace('"' , '\"' ,trim($title));


	/* First do a title search */
	$topics = $wpdb->get_results('SELECT * FROM '.$wpdb->posts.' WHERE post_title LIKE "%' . $title .'%" AND post_type="topic" AND post_status="publish"' );
